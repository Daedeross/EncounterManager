namespace Foundation.ServiceFabric
{
    ...
    [EventSource(Name = "Insignia-Foundation-Services")]
    public sealed class ServiceEventSource : EventSource
    {
        ...
        {
            public const EventKeywords ServiceInitialization = (EventKeywords)0x1L;
            public const EventKeywords Diagnostic = (EventKeywords)0x2L;
            public const EventKeywords Performance = (EventKeywords)0x4L;
            public const EventKeywords Requests = (EventKeywords)0x8L;
        }
        #endregion
        ...
        [Event(ServiceMessageEventId, Level = EventLevel.Informational, Message = "{0}: {7}")]
        private
#if UNSAFE
        unsafe
#endif
        ...
        #region ServiceException

        [NonEvent]
        public void ServiceException(StatelessService service, Exception exception)
        {
            if (!IsEnabled()) return;
        ...
        private const int ExceptionEventId = 8;
        [Event(ExceptionEventId, Level = EventLevel.Error, Message = "{0}:{1} threw an exception", Keywords = Keywords.Diagnostic)]
        private void ServiceException(string serviceName, string identity, string partition, string exception)
        {
            if (!IsEnabled()) return;

            WriteEvent(ExceptionEventId, serviceName, identity, partition, exception);
        }

        #endregion ServiceException
        #region ServiceError

        [NonEvent]
        public void ServiceError(StatelessService service, string message, params object[] args)
        {
            if (!IsEnabled()) return;
        ...
        private const int ErrorEventId = 9;
        [Event(ErrorEventId, Level = EventLevel.Error, Message = "{0}:{1} had a non-fatal error", Keywords = Keywords.Diagnostic)]
        private void ServiceError(string serviceName, string identity, string partition, string message)
        {
            if (!IsEnabled()) return;

            WriteEvent(ExceptionEventId, serviceName, identity, partition, message);
        }
